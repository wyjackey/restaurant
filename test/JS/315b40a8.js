! function() {
	var a = function() {"use strict";
		...
		var a, b = {
			on : function(a, b, c) {
				...
				"string" == typeof a && b && "function" == typeof b && (d[a] || (d[a] = []), d[a].push({
					callback : b,
					context : c
				}))
			},
			...
			trigger : function(a) {
				...
				if (!e || 0 === e.length)
					return
					void (window.console && "function" == typeof window.console.log && console.log("no one subscribe topic", a));
				for ( b = e.length; b--; )
		...
		var b, c = {
			request : function(b) {
				return a(b), b.data = b.data || {}, ELEME.csrfToken && (b.data.csrf_token = ELEME.csrfToken), $.ajax(b).then(function(a, b, c) {
					if (arguments.length) {
						var d = $.Deferred();
						return "ok" === a.status ? d.resolveWith(null, [a.data, b, c]) : "error" === a.status ? d.rejectWith(null, [a.error, b, c]) :
						void 0
					}
				}, function(a, b, c) {
					var d = $.Deferred(), e = {
					...
					return d.rejectWith(null, [e, b, a])
				})
			}
		};
		return b = c
		...
		var a, b = window.devicePixelRatio > 1 ? 2 : 1, c = function() {
			var a = $.Deferred(), b = new Image;
			return b.onload = function() {
				a.resolve(!0)
			}, b.onerror = function() {
				a.resolve(!1)
			}, b.src = "data:image/webp;base64,UklGRkoAAABXRUJQVlA4WAoAAAAQAAAAAAAAAAAAQUxQSAsAAAABBxAREYiI/gcAAABWUDggGAAAADABAJ0BKgEAAQABABwlpAADcAD+/gbQAA==", a.promise()
		}, d = c(), e = function(a, c, e) {
			return d.then(function(d) {
				var f = d ? "/format/webp/quality/85" : "", g = a + "?imageMogr2" + f, h = "/thumbnail/" + c + "x" + e, i = "/thumbnail/" + 2 * c + "x" + 2 * e;
				return b > 1 ? g + i : g + h
			})
		};
		return a = e
	}();
	! function(a, b, c, d, e, f, g, h, i) {"use strict";
		var j = a, k = b, l = c, m = d, n = e, o = f, p = g, q = h, r = i;
		"undefined" == typeof Eleme && (window.Eleme = {}), Eleme.Common = {
			RenderBuffer : j,
			Evt : k,
			Util : m,
			Time : q,
		...
		if ($(".eleme_dropdown:not(#topbar_msg, #topbar_cart)").uDropdown(), $('input:not([type="password"]), textarea').placeholder(), $('[data-toggle="bs-tooltip"]').tooltip({
			animation : !1,
			placement : "top",
			delay : {
				show : 700,
			...
			var a = $("#mod_fixed");
			Eleme.Component.scrollspy("fixedMod", {
				min : 300,
				onEnter : function() {
		...
		var b = {
			feedbackUrl : "/feedback/addSuggestion",
			feedbackModalSelector : "#modal_feedback",
			feedbackInputSelector : "#feedback_input",
			feedbackFormSelector : "#feedback_form",
			feedbackErrorSelector : ".feedback_err_tip",
			cancelFeedbackSelector : ".cancel_feedback",
			submitFeedbackSelector : ".submit_feedback"
		};
		$(b.feedbackFormSelector).on("submit", function(a) {
			...
			void  $(b.feedbackErrorSelector).html("亲爱的用户，您还没有添加反馈意见").removeClass("hide") : ($(b.submitFeedbackSelector).html("正在提交"),
			void  Eleme.Common.Http.request({
				url : c,
				type : "get",
				dataType : "jsonp",
				data : {
			...
			$.fn.srcset = function() {
				return $(this).each(function(d, e) {
					var f = $(e), g = f.attr("data-srcset"), h = +f.attr("width"), i = +f.attr("height");
					f.removeAttr("data-srcset"), Eleme.Common.optimizeImgUrl(g, h, i).then(function(d) {
						a.push([f, d]), c(), e.onload = function() {
							b--, c()
						}, e.onerror = function() {
							b--, c()
						}
	...
	var a = function() {"use strict";
		...
		var b, c = Eleme.Common.Http, d = Eleme.Common.PubSub, e = {
			searchPanelSelector : "#topbar_search",
			searchFormSelector : "#tsearch_form",
			resaultWrapSelector : "#tsearch_autocomplete",
			localLabelSelector : ".ts_inRst",
			removeLocalLabelSelector : "#ts_remove",
			searchInputSelector : "#tsearch_input",
			singleFoodSelector : ".tsearch-item",
			loadingSpinSelector : "#ts_loading",
			iconSearchSelector : "#icon_tsearch",
			searchClearSelector : "#ts_clear",
			filterFoodTopic : "filterFood",
			searchSuccessTopic : "restaurantSearchSuccess",
			searchFailureTopic : "restaurantSearchFailure",
			foodScrollTopic : "foodScroll",
			emptyTip : '<div class="tsearch-no-result">没有找到相关的美食</div>',
			foodContainer : ['<div class="group">', '<span class="tsearch-cate">美食</span>', '<ul class="tsearch-list"></ul>', "</div>"].join(""),
			restaurantContainer : ['<div class="group">', '<span class="tsearch-cate">餐厅</span>', '<ul class="tsearch-list"></ul>', "</div>"].join(""),
			inRestaurantTpl : ['<span class="ts_inRst tsearch-in-rst">', '<i class="glyph-search"></i>搜本店<a id="ts_remove" class="action-remove">×</a>', "</span>"].join(""),
			foodTpl : ['<li class="tsearch-item ts-dish">', '<a class="tsd-name" data-foodid="<%- data.id %>"><%- data.name %></a>', '<span class="tsd-price"><span class="symbol-rmb"></span><%- data.price %></span>', "</li>"].join(""),
			globalFoodTpl : ['<li class="tsearch_item tsearch-item ts-dish">', '<a class="tsd-name" href="<%- "//" + "r" + "." + ELEME.rootHost + "/" + data.restaurantNameForUrl + "#food/" + data.id %>" target="_blank"><%- data.name %></a>', '<a class="tsd-rst" href="<%- "//" + data.restaurantNameForUrl + "." + ELEME.rootHost %>" target="_blank"><%- data.restaurantName %></a>', '<span class="tsd-price">', '<span class="symbol-rmb"><%- data.price %></span>', "</span>", "</li>"].join(""),
			globalRestaurantTpl : ['<li class="tsearch_item">', '<a class="tsearch-item ts-restaurant" href="<%- "//" + "r" + "." + ELEME.rootHost + "/" + data.nameForUrl %>" target="_blank">', '<img class="tsr-logo" src="<%- ELEME.fussHost + data.logo %>" alt="" />', '<span class="tsr-name"><%- data.name %></span>', '<% if (data.deliverTime) { %><span class="tsr-time"><%- data.deliverTime %>分钟</span><% } %>', "</a>", "</li>"].join(""),
			local : !1,
			activeClass : "ui_open",
			searchUrl : "/search/suggestion",
			restaurantTag : "restaurant_show"
		};
		return a.prototype.init = function() {
			...
			}, this), d.subscribe(e.searchFailureTopic, function(a) {
			})
			...
			this.$el.on("keyup", e.searchInputSelector, $.proxy(this.handleType(), this)), this.$el.on("click", e.removeLocalLabelSelector, $.proxy(this.changeToNomal, this)), this.$el.on("click", e.singleFoodSelector, function(b) {
				...
				return d.hasClass(e.activeClass) ? !1 : (d.addClass(e.activeClass), c.addClass("focus"),
				void  b.stopPropagation())
			}), this.$el.find(e.searchFormSelector).on("submit", function() {
				var a = $(this).find(e.searchInputSelector).val();
				a = encodeURIComponent(a);
				var b = $("meta[name=geohash]").attr("content");
				return window.location.href = ELEME.formerBaseUrl + "/place/" + b + "/search/" + a, !1
			})
			...
			return function() {
				...
				a !== $.trim(b) && (this.local ? d.publish(e.filterFoodTopic, {
					keyword : b
				}, $.proxy(this.parseResault, this)) : this.search({
					wd : b
				}), a = b)
			}
		}, a.prototype.search = function(a) {
			var b = "//" + ELEME.mainHost + e.searchUrl;
			this.$el.find(e.loadingSpinSelector).removeClass("hide").siblings(e.searchClearSelector).addClass("hide"), c.request({
				type : "get",
				url : b,
				data : a,
				dataType : "jsonp"
			}).done(function(a, b, c) {
				d.publish(e.searchSuccessTopic, a), a.foods.length > 0 || a.restaurant.length > 0 ? ga("send", "event", "topbarSearch", "hasResult", "global", a.foods.length + a.restaurant.length) : ga("send", "event", "topbarSearch", "noResult", "global", 0)
			}).fail(function(a) {
		...
		var b, c = Eleme.Common.Util, d = Eleme.Common.Http, e = Eleme.Common.PubSub, f = {
			topbarCartSelector : "#topbar_cart",
			cartWrapSelector : "#tcart_wrapper",
			cartLoadingSelector : "#tcart_loading",
			cartContentSelector : ".t_c_c",
			rstPhoneSelector : ".rst_header_tel",
			cartTotalSelector : "#tcart_total",
			basketSyncSuccessTopic : "basketSyncSuccess",
			basketSyncFailureTopic : "topBasketSyncFailure",
			foodTpl : ['<li class="tcart-item">', '<span class="tcart-dish"><%- data.name %></span>', '<span class="tcart-amount">× <%- data.quantity %></span>', '<span class="tcart-price">', '<span class="symbol-rmb"><%= data.foodTotal %></span>', "</span>", "</li>"].join(""),
			cartInfoTpl : ['<h3 class="tcart-title group">', '<a class="tcart-restaurant" href="<%- data.restaurantNameForUrl %>"><% if (data.restaurantName.length > 10) { %><%- data.restaurantName.substr(0, 9) %>&hellip;<% } else { %><%- data.restaurantName %><% } %></a>', '<span class="tcart-cost"><span class="symbol-rmb"><%- data.total %></span></span>', "</h3>", '<div class="tcart-content t_c_c"></div>'].join(""),
			checkBtnTpl : ['<div class="twidget-footer">', "<% if (data.deliverPlus >= 0) { %>", "<% if (data.phoneOrder) { %>", '<p class="tcart-hint" title="该餐厅当前只支持电话订餐">餐厅电话：<span class="tcart-tel"><%- data.phone %></span></p>', "<% } else { %>", '<a id="tcart_checkout" class="twidget-btn" href="<%- data.url %>">去结算</a>', "<% } %>", "<% } else { %>", '<p class="tcart-hint">还需要 <span class="tcart-remainder"><span class="symbol-rmb"></span><%= -data.deliverPlus %></span> 起送</p>', "<% } %>", "</div>"].join(""),
			singleCartTpl : '<h4 class="tcart-basket"><%- data.basketId %>号篮子</h4>',
			emptyCartHintTpl : '<div class="twidget-empty">篮子空空，肚子空空，快去订餐吧~</div>',
			cartFetchUrl : "/cart"
		};
		return a.prototype.init = function() {
			...
			d.request({
				...
				e.publish(f.basketSyncSuccessTopic, a, {
					top : !0
				})
			}).fail(function(a) {
			...
			var b, c = a.groups;
			return this.$el.find(f.cartWrapSelector).empty(), this.$el.find(f.cartLoadingSelector + "," + f.cartWrapSelector).hide(), 0 === c.length ?
			void  this.showEmptyTip() : (a.restaurantNameForUrl = "//" + a.restaurantNameForUrl + "." + ELEME.rootHost, this.appendRstHead(a), this.appendCartBody(c), 1 === c.length && this.$el.find(f.cartContentSelector + " h4").remove(), a.phoneOrder && ( b = this.getPhoneNumber()), this.appendCheckoutBtn({
				url : "http://" + ELEME.mainHost + "/cart/checkout",
				deliverAmount : a.deliverAmount,
				restaurantDeliverAmount : a.restaurantDeliverAmount,
				phoneOrder : a.phoneOrder,
				total : a.total,
				phone : b
			}),
			void  this.$el.find(f.cartWrapSelector).show())
			...
			for ( b = 0, g = a.length; g > b; b++) {
				for ( h = a[b], k = $(n({
					basketId : b + 1
				})), l = $('<ul class="tcart-list"></ul>'), d = h.length; d--; )
		...
		var b, c = Eleme.Common.Http, d = Eleme.Common.PubSub, e = {
			topbarMsgSelector : "#topbar_msg",
			msgTotalSelector : "#tmsg_total",
			msgContainerSelector : "#tmsg_wrapper",
			msgLoadingSelector : "#tmsg_loading",
			clearBtnSelector : "#tmsg_clear",
			singleMsgSelector : ".tmsg_item",
			msgFetchErrorTopic : "msgFetchError",
			msgFetchDoneTopic : "msgFetchDone",
			unreadMsgTopic : "unreadMsg",
			userAuthTopic : "userAuth",
			msgTpl : ['<a class="tmsg-item tmsg_item" data-id="<%- data.id %>" href="<%- data.url %>" target="_blank">', '<h4 class="tmsg-title" title="<%- data.abstract %>"><%- data.abstract %></h4>', '<span class="tmsg-time"><%- data.createdAt %></span>', '<p class="tmsg-content"><%- data.content %></p>', "</a>"].join(""),
			noMsgHintTpl : '<div id="tmsg_empty" class="twidget-empty">没有新信息</div>',
			msgClearBtnTpl : '<div class="twidget-footer"><a id="tmsg_clear" class="twidget-btn">知道了</a></div>',
			msgUrl : "/topbar/message",
			msgReadUrl : "/topbar/read/message",
			syncTimeStamp : 6e6
		};
		return a.prototype.init = function() {
			...
			a = window.localStorage.getItem("updateTimeStamp"), b = window.localStorage.getItem("unread"), g - a > e.syncTimeStamp ? c.request({
				type : "get",
				url : h,
				dataType : "jsonp"
			}).done(function(a) {
			...
			if (f.empty(), this.storeToLocal(this.currentUnread), d.publish(e.unreadMsgTopic, a.length), this.active = !0, this.$el.find(e.msgLoadingSelector).hide(), 0 === a.length)
				return
				void  f.append(e.noMsgHintTpl);
			for ( b = a.length; b--; )
				f.append(c(a[b]));
			f.append(e.msgClearBtnTpl)
		}, a.prototype.markAsRead = function(a) {
			var b, d, f, g = this;
			a && ( d = a.payLoad, f = a.success, d && f && "function" == typeof f && ( b = "//" + ELEME.mainHost + e.msgReadUrl + d, c.request({
				type : "get",
				url : b,
				dataType : "jsonp"
			}).done(function(b, c, d) {
		...
		}, a.prototype.showErrorTip = function(a) {
		...
		var b, c = Eleme.Common.Http, d = Eleme.Common.PubSub, e = {
			topbarWrapSelector : "#topbar",
			cartTotalSelector : "#tcart_total",
			msgTotalSelector : "#tmsg_total",
			userNameSelector : "#t_username",
			userNameWrapSelector : "#topbar_username",
			loginWrapSelector : "#topbar_logreg",
			msgWrapSelector : "#topbar_msg",
			cartWrapSelector : "#topbar_cart",
			dpLinkSelector : "#tdp_link",
			logoutSelector : "#topbar_logout",
			userAuthTopic : "userAuth",
			userInfoDoneTopic : "userInfoDone",
			userInfoUrl : "/topbar/userinfo",
			logoutUrl : "http://" + ELEME.accountHost + "/logout"
		};
		return a.prototype.init = function() {
